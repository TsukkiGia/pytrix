...
class Ship(GSprite):
    ...
    # SHOOT_SOUND = Sound(SHIP_PEW)
    # BLAST_SOUND = Sound(BLAST)
    # GETTERS AND SETTERS (ONLY ADD IF YOU NEED THEM)

    # INITIALIZER TO CREATE A NEW SHIP
    def __init__(self, x, bottom, width, height, source, format):
        super().__init__(x = x,
                         bottom = bottom,
                         width = width,
                         height = height,
    ...
    def collides(self,bolt):
        ...
        """
        corners = [(bolt.left, bolt.top), (bolt.right, bolt.top), (bolt.left, bolt.bottom), (bolt.right, bolt.bottom)]
...
class Alien(GSprite):
    ...
    """
    #  IF YOU ADD ATTRIBUTES, LIST THEM BELOW
    # Attribute type: the index of the alien in ALIEN_IMAGES
    # Invariant: type is an integer in [0,1,2]

    # SHOOT_SOUND = Sound(ALIEN_PEW)
    # GETTERS AND SETTERS (ONLY ADD IF YOU NEED THEM)

    # INITIALIZER TO CREATE AN ALIEN
    def __init__(self, x, y, width, height, type, format):
        super().__init__(x=x, y=y, width=width, height=height, source = ALIEN_IMAGES[type], format = format)
    ...
    def collides(self,bolt):
        ...
        Precondition: bolt is of class Bolt
        """
        corners = [(bolt.left, bolt.top),
                   (bolt.right, bolt.top),
                   (bolt.left, bolt.bottom),
                   (bolt.right, bolt.bottom)]
