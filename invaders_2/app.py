...
class Invaders(GameApp):
    ...
    def start(self):
        ...
        """
        # IMPLEMENT ME
        self._background = GRectangle(x=GAME_WIDTH/2,
                                      y=GAME_HEIGHT/2,
                                      width=GAME_WIDTH,
                                      height=GAME_HEIGHT,
                                      fillcolor='black')
        ...
        self.lastkeys = 0
        self._wave = None
        self._theme = Sound("Theme.mp3")
        self._wavetheme = Sound("Active.mp3")



        #Welcome Screen
        if self._state is STATE_INACTIVE:
            self._theme.play(loop=True)
            ...
            self._shipImage =  GImage(x=GAME_WIDTH/2,
                                      y=GAME_HEIGHT/2,
                                      width = GAME_WIDTH,
                                      height = GAME_HEIGHT,
                                      source = GAME_WELCOME)
    ...
    def update(self,dt):
        ...
        # creating a new wave of aliens and handle state changes
        self._newWave()
        self._active(dt)
        self._paused()
        self._completed()
    ...
    def draw(self):
        ...
        if self._text is not None:
            if self._state == STATE_INACTIVE:
                self._shipImage.draw(self.view)
    ...
    def _determineState(self):
        ...
        if self._wave is not None:
            if self._wave.getLives() > 0:
                if self._wave.getShipStateA() == True:
                    self._state = STATE_PAUSED
                else:
                    self._state = STATE_ACTIVE
            if self._wave.getAnnihilation() or self._wave.getBreach() or self._wave.getLives() < 1:
                self._state = STATE_COMPLETE

        curr_keycount = self.input.key_count
        curr_keys = self.input.keys
        # Only change if we have just pressed the keys this animation frame
        change = curr_keycount > 0 and 's' in curr_keys and self.lastkeys == 0
        if change and self._state == STATE_INACTIVE:
            # Click happened.  Change the state to newwave
            ...
            if self._wave.getLives() > 0:
                ...
                self._text = None
                self._wave.setShip()
                self._wave.setLives(1)
        # Update last_keys
        self.lastkeys = curr_keycount

        # turn off theme songs when welcome screen is gone
        if self._state != STATE_INACTIVE:
            self._theme.volume=0.0


    def _newWave(self):
        """ Changes state to STATE_ACTIVE to begin a wave """
        if self._state == STATE_NEWWAVE:
            self._wave = Wave()
            self._wavetheme.play(loop=True)
            self._wavetheme.volume=0.5
            self._state = STATE_ACTIVE


    def _active(self, dt):
        """ Runs update function from Wave if state is STATE_ACTIVE """
        if self._state == STATE_ACTIVE:
            self._wave.update(self.input, dt, self.lastkeys)


    def _paused(self):
        """ Displays text if state is STATE_PAUSED """
        if self._state == STATE_PAUSED:
            self._text = GLabel(text="Press 'S' to Continue\nLives: {}".format(self._wave.getLives()-1),
                                font_size=50,
                                font_name = 'Arcade.ttf',
                                x = GAME_WIDTH/2,
                                y = GAME_HEIGHT/2,
    ...
    def _completed(self):
        """ Displays text depending on what condition triggers state to STATE_COMPLETE """

        if self._state == STATE_COMPLETE:
            self._wavetheme.volume=0.0

            if self._wave.getBreach():
                self._text = GLabel(text="Game Over\nAliens Breached the Atmosphere\nYou Lose!",
                                    font_size=50,
                                    font_name = 'Arcade.ttf',
                                    x = GAME_WIDTH/2,
                                    y = GAME_HEIGHT/2,
                                    linecolor="yellow")
            if self._wave.getAnnihilation():
                self._text = GLabel(text="Threat Averted\nYou Win!\n\nScore: {}".format(self._wave.getScore()),
                                    font_size=50,
                                    font_name = 'Arcade.ttf',
                                    x = GAME_WIDTH/2,
                                    y = GAME_HEIGHT/2,
